...
execute 'set_root_auth_strategy' do
  command "mysql -u root -D #{node[:seconion][:server][:sguil_server_name]}_db -e \"ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY '';\" && touch /root/mysql_native_#{ node[:seconion][:server][:sguil_server_name] }"
  action :run
  creates "/root/mysql_native_#{ node[:seconion][:server][:sguil_server_name] }"
end
